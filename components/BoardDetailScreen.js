...
class BoardDetailScreen extends Component {
  ...
  componentDidMount() {
    ...
    // fetch from the firebase database
    ref.get()
      .then((doc) => {
        // if the doc existe then we will pass the data to the state
        // and we will set isLoading to false
        if (doc.exists) {
          this.setState({
            board: doc.data(),
            key: doc.id,
  ...
  // delete a board from the firebase store database
  deleteBoard(key) {
    const { navigation } = this.props;
